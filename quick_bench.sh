...
N=100000000
U=8
common_stmt="from pyroaring import BitMap as pybitmap; from roaringbitmap import RoaringBitmap as cybitmap; import random; my_set="
...
for stmt in "s1.add(val)" "val in s1" "x=list(s1)" "s1==s3" "s1 <= s3" "x=s1|s2" "x=s1&s2" "x=s1^s2"; do
    echo "$stmt"
    for cls in $classes; do
        t=`python3 -m timeit -u msec -s "$common_stmt$cls$init_stmt" "$stmt" | cut -d" " -f6`
        echo "    $cls $t"
