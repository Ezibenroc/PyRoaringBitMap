...
cdef class BitMap64(AbstractBitMap64):
    ...
    def add_checked(self, uint64_t value):
        """
        Add an element to the bitmap. This raises a KeyError exception if the element is already present.
        ...
        KeyError: 42
        """
        cdef bool test = croaring.roaring64_bitmap_add_checked(self._c_bitmap, value)
        if not test:
            raise KeyError(value)
    ...
    def update(self, *all_values): # FIXME could be more efficient
        ...
        cdef uint64_t[:] buff
        for values in all_values:
            if isinstance(values, AbstractBitMap64):
                self |= values
            elif isinstance(values, range):
                if len(values) == 0:
                    continue
                ...
                if step == 1:
                    ...
                    self |= AbstractBitMap64(values)
            elif isinstance(values, array.array) and len(values) > 0:
                buff = <array.array> values
                croaring.roaring64_bitmap_add_many(self._c_bitmap, len(values), &buff[0])
            else:
                try:
                    size = len(values)
                except TypeError:  # object has no length, creating a list
    ...
    def discard(self, uint64_t value):
        """
        Remove an element from the bitmap. This has no effect if the element is not present.
        ...
        BitMap64([12])
        """
        croaring.roaring64_bitmap_remove(self._c_bitmap, value)
    ...
    def remove(self, uint64_t value):
        ...
        KeyError: 3
        """
        cdef bool test = croaring.roaring64_bitmap_remove_checked(self._c_bitmap, value)
        if not test:
            raise KeyError(value)

    cdef binary_iop(self, AbstractBitMap64 other, (void)func(croaring.roaring64_bitmap_t*, const croaring.roaring64_bitmap_t*) noexcept) noexcept:
        func(self._c_bitmap, other._c_bitmap)
        return self

    def __ior__(self, other):
        if self._c_bitmap == (<AbstractBitMap64?>other)._c_bitmap:
            return self
        return (<BitMap64>self).binary_iop(<AbstractBitMap64?>other, croaring.roaring64_bitmap_or_inplace)

    def __iand__(self, other):
        if self._c_bitmap == (<AbstractBitMap64?>other)._c_bitmap:
            return self
        return (<BitMap64>self).binary_iop(<AbstractBitMap64?>other, croaring.roaring64_bitmap_and_inplace)

    def __ixor__(self, other):
        if self._c_bitmap == (<AbstractBitMap64?>other)._c_bitmap:
            self.clear()
            return self
        return (<BitMap64>self).binary_iop(<AbstractBitMap64?>other, croaring.roaring64_bitmap_xor_inplace)

    def __isub__(self, other):
        if self._c_bitmap == (<AbstractBitMap64?>other)._c_bitmap:
            self.clear()
            return self
        return (<BitMap64>self).binary_iop(<AbstractBitMap64?>other, croaring.roaring64_bitmap_andnot_inplace)
    ...
    def difference_update(self, *others):
        ...
        BitMap64([5])
        """
        self.__isub__(AbstractBitMap64.union(*others))
    ...
    def pop(self):
        ...
        try:
            ...
            raise KeyError('pop from an empty BitMap64')
        self.remove(value)
        return value
    ...
    def add_range(self, uint64_t range_start, uint64_t range_end):
        ...
        if range_end <= range_start or range_end == 0:
            return
        croaring.roaring64_bitmap_add_range(self._c_bitmap, range_start, range_end)
    ...
    def flip_inplace(self, uint64_t start, uint64_t end):
        ...
        """
        croaring.roaring64_bitmap_flip_inplace(self._c_bitmap, start, end)
    ...
    def remove_range(self, uint64_t range_start, uint64_t range_end):
        ...
        if range_end <= range_start or range_end == 0:
            return
        croaring.roaring64_bitmap_remove_range(self._c_bitmap, range_start, range_end)